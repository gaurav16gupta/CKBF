...
BloomFilter::BloomFilter(uint32_t sz, uint32_t k)
  : bits(new atomic<uint8_t>[sz >> 3]), size(sz), k(k) {

}
void BloomFilter::insert(uint32_t *hashes) {
  // TODO: hardcode for loop
  for (uint32_t i = 0; i < k; ++i) {
        auto value =  bits[hashes[i] >> 3].load();
        auto new_value = value|  1 << (hashes[i] & 7);
        while(!bits[hashes[i]>>3].compare_exchange_weak(value, new_value)) {
            new_value = value |  1 << (hashes[i] & 7);
        }
  }
}
